...
Major sections receive stronger spacing and open-rule landmarks than item
interiors; labels stay close to their elaboration. No author layout fields or
prose shortening are needed to request this hierarchy.
Title, provenance and scope remain visible. The relationship and its interpretation
caveat label the sequence itself and scroll with it, rather than competing with
the title. Offscreen context is available, not necessarily visible. Scrolling snaps to
paragraph starts when a paragraph fits the body, avoiding a misleading leading
fragment. Oversized paragraphs can scroll internally, with an explicit earlier-text
warning; the footer marks a paragraph cut at the bottom as continuing. These are
incomplete excerpts, not standalone claims. Even complete detail can depend on
its label or preceding context. Overflow is explicit; no silent shortening or live
current/completed status. At less than
...
# Offset is a body-line index, snapped to a legal paragraph/fragment position.
# Large values clamp to the last view; it may leave a few blank rows.
...
Live: j/k or arrows move between legal positions; PgUp/PgDn page without skipping
unread text (overlap where possible); Home/End jump; q/Esc/Ctrl-C quits. Edits require relaunch;
