...
> **POST-TRIAL discovery-only correction:** this revision was made after the
> frozen fresh-author run. It was not supplied to that author, has not been
> trialed, and does not change the trial evidence or product approval. The exact
> pre-correction entrypoint and source are archived under
> `docs/assets/compare/trial/frozen-before-discovery-correction/`.
...
- **Sequence:** supplied procedure or chronology where order itself matters. Do
  not turn alternatives sorted by price into a numbered journey. For actual
  Sequence authoring, use the unchanged [Sequence author entrypoint](sequence.md).
...
### Supported experiment limits

These are the implemented example limits, not production-API commitments:

- The UTF-8 input is at most **65,536 bytes (64 KiB)**.
- `title`, `provenance`, and `scope` are each at most **60 terminal display
  cells**. `criterion.name` is at most **40 cells** and `alternative.name` at
  most **24 cells**.
- Every other display string (`intent`, criterion questions, answer values,
  criterion references, and qualifications) is nonblank and at most **500
  Unicode scalar values**. Display text cannot contain control or bidi-control
  characters.
- There are **2–8 criteria** and **2–6 alternatives**. Each alternative has
  exactly one answer for every criterion: therefore **exactly 2–8 answers per
  alternative**, with no duplicate or missing criterion names.
- A `known` answer requires a nonblank `value`. `unknown` and `unavailable`
  answers must omit `value`; their optional `qualification` remains separate
  display text. Unknown fields are rejected at every object level.
- Headless preview accepts **1–240 columns × 1–100 rows**. Live Compare asks for
  at least **64 columns × 12 rows**; smaller live areas show a resize message.

The limits above are checked at the parsing/rendering boundary. They do not
validate factual truth, comparability, appropriate idiom selection, or human
readability.

### Minimal complete document

This is a complete valid two-criterion/two-alternative document. It demonstrates
shape and explicit unknown state only; it is not a recommended decision or trial
fixture:

```json
{
  "title": "Choose a room",
  "provenance": "Fictional; not live or independently verified.",
  "scope": "One meeting; notes supplied 10 June 2026; USD.",
  "intent": "Help an organizer inspect two options; no ranking or booking.",
  "criteria": [
    {"name": "Capacity", "question": "What capacity is supplied?"},
    {"name": "Fee", "question": "What room fee is supplied?"}
  ],
  "alternatives": [
    {
      "name": "North Hall",
      "answers": [
        {"criterion": "Capacity", "state": "known", "value": "40 seats"},
        {"criterion": "Fee", "state": "known", "value": "100 USD"}
      ]
    },
    {
      "name": "South Hall",
      "answers": [
        {"criterion": "Capacity", "state": "known", "value": "32 seats"},
        {"criterion": "Fee", "state": "unknown", "qualification": "Fee is not supplied."}
      ]
    }
  ]
}
